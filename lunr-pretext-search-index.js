...
},
{
  ...
  "body": " 7.1 Diagonalization of Symmetric Matrices   7.1 Diagonalization of Symmetric Matrices   Overview   Topics   Symmetric matrices  Orthogonal diagonalization  Spectral decomposition     Goals   Construct an orthogonal diagonalization of a symmetric matrix.  Construct a spectral decomposition of a matrix.     Symmetric Matrix   A matrix is symmetric if .      Which of the following matrices are symmetric? Let and represent real numbers.       Matrices and are symmetric. Matrix is symmetric if and only if . Matrix is not symmetric. In fact, a non-square matrix cannot possibly be symmetric.       Is Symmetric   For any matrix with columns , the matrix is symmetric and has entries given by dot products of the columns of .    Note that . Thus, is symmetric.     Orthogonality of Eigenspaces   If is symmetric and are eigenvectors corresponding to distinct eigenvalues, then and are orthogonal .  More generally, eigenspaces associated with distinct eigenvalues are orthogonal subspaces.    Suppose and suppose are eigenvectors of corresponding to distinct eigenvalues , respectively. Thus, and .  Our goal is to show that and are orthogonal. Therefore, we want to show that . Note the following: Thus, we have that . Subtracting the right side expression from both sides yields Since , we have that . Therefore, it must be the case that Hence, and are orthogonal.      Example 1: Orthogonal Diagonalization    Diagonalize using an orthogonal matrix. The eigenvalues of are given.     Let's start with finding the -eigenspace of . Note that . Since has two free variables, the -eigenspace is two dimensional. If we were only trying to diagonalize , then any basis of the -eigenspace would be fine. But we want to diagonalize so that is an orthogonal matrix. Therefore, we want to find an orthonormal basis of the -eigenspace . Thus, we choose the basis vectors .  Now, let's find a basis for the -eigenspace . Note that . Since has only a single free variable, any basis we choose will be an orthogonal basis, we will just make sure to use a normalized vector as a column of . Let . Note that this vector is guaranteed to be orthogonal to and by .  It follows that a diagonalization of is given by , where . Since is orthogonal, we have that , so this diagonalization could be written . We call this diagonalization using an orthogonal matrix an orthogonal diagonalization .       Spectral Theorem    Recall that if is an orthogonal matrix, then .     Spectral Theorem   If is an  symmetric matrix, then:   All eigenvalues of are real.  The dimension of each eigenspace equals its algebraic multiplicity.  Eigenspaces corresponding to distinct eigenvalues are orthogonal.   can be orthogonally diagonalized as , where is orthogonal.     Note that only symmetric matrices can be orthogonally diagonalized. This is because if is a matrix that can be diagonalized as , then , so is symmetric.      Spectral Decomposition   Suppose can be orthogonally diagonalized as . Then has the decomposition . Note that each term is a rank 1 matrix.        Construct a spectral decomposition of , given its orthogonal diagonalization:     Using the formula in , we have that . Note that the first term in the sum is a fairly close approximation of . In fact, it is the best rank 1 approximation of .         Note in the solution to that if is the larger of the two eigenvalues and is a normalized eigenvector corresponding to , then is a close approximation of the matrix . In fact, it is the closest approximation of using a rank 1 matrix.  In general, if we label the eigenvalues of an symmetric matrix in non-increasing order by magnitude, so , then each matrix is the closest rank approximation of , which gets more accurate as approaches .   : The distance between two matrices is measured by subtracting the two matrices and calculating the square root of the sums of the squares of the entries in the resulting matrix. This calculation is the same calculation used to calculate the distance between two vectors. By closest, we mean that there is no rank matrix that is closer. There could be other rank matrices that are as close.      Construct a spectral decompositon of .    Using the information from the solution to , we have   Since every eigenvalue has the same magnitude, each term of the Spectral Decomposition will be an equivalent rank 1 approximation according to the metric described above. For example, note that using the first term of the decomposition as an approximation results in the following error: . Using the third term of the decomposition as an approximation results in the following error: .  Adding any two of the three terms would lead to a best rank 2 approximation of . Note that the sum of the last two terms of our Spectral Decomposition yields the matrix , which is away from according to the metric described above.      "
},
{
...
},
{
  ...
  "number": "7.13",
  "title": "",
  "body": "  Which of the following matrices are symmetric? Let and represent real numbers.       Matrices and are symmetric. Matrix is symmetric if and only if . Matrix is not symmetric. In fact, a non-square matrix cannot possibly be symmetric.   "
},
{
  ...
  "number": "7.14",
  "title": "",
  "body": " For any matrix with columns , the matrix is symmetric and has entries given by dot products of the columns of .    Note that . Thus, is symmetric.   "
},
{
  ...
  "body": " Orthogonality of Eigenspaces   If is symmetric and are eigenvectors corresponding to distinct eigenvalues, then and are orthogonal .  More generally, eigenspaces associated with distinct eigenvalues are orthogonal subspaces.    Suppose and suppose are eigenvectors of corresponding to distinct eigenvalues , respectively. Thus, and .  Our goal is to show that and are orthogonal. Therefore, we want to show that . Note the following: Thus, we have that . Subtracting the right side expression from both sides yields Since , we have that . Therefore, it must be the case that Hence, and are orthogonal.   "
},
{
  "id": "ex-7-1-orth-diag",
  "level": "2",
  ...
  "number": "7.16",
  "title": "",
  "body": "  Diagonalize using an orthogonal matrix. The eigenvalues of are given.     Let's start with finding the -eigenspace of . Note that . Since has two free variables, the -eigenspace is two dimensional. If we were only trying to diagonalize , then any basis of the -eigenspace would be fine. But we want to diagonalize so that is an orthogonal matrix. Therefore, we want to find an orthonormal basis of the -eigenspace . Thus, we choose the basis vectors .  Now, let's find a basis for the -eigenspace . Note that . Since has only a single free variable, any basis we choose will be an orthogonal basis, we will just make sure to use a normalized vector as a column of . Let . Note that this vector is guaranteed to be orthogonal to and by .  It follows that a diagonalization of is given by , where . Since is orthogonal, we have that , so this diagonalization could be written . We call this diagonalization using an orthogonal matrix an orthogonal diagonalization .   "
},
{
...
},
{
  ...
  "body": " Spectral Theorem   If is an  symmetric matrix, then:   All eigenvalues of are real.  The dimension of each eigenspace equals its algebraic multiplicity.  Eigenspaces corresponding to distinct eigenvalues are orthogonal.   can be orthogonally diagonalized as , where is orthogonal.     Note that only symmetric matrices can be orthogonally diagonalized. This is because if is a matrix that can be diagonalized as , then , so is symmetric.   "
},
{
...
},
{
  ...
  "number": "7.20",
  "title": "",
  "body": "  Construct a spectral decomposition of , given its orthogonal diagonalization:     Using the formula in , we have that . Note that the first term in the sum is a fairly close approximation of . In fact, it is the best rank 1 approximation of .   "
},
{
...
},
{
  ...
  "body": " 7.2 Quadratic Forms   7.2 Quadratic Forms   Overview   Topics   Quadratic forms  Change of variables  Principal axes theorem  Classification of quadratic forms     Goals   Characterize and classify quadratic forms using eigenvalues and eigenvectors.  Express quadratic forms in the form .  Apply the principal axes theorem to remove cross-product terms.    Motivating Question  Does the inequality below hold for all ?     Quadratic Form   A quadratic form is a function defined by   where is an  symmetric matrix.      Example 1    Compute and for the following matrices.          Note that            Note that      The graphs of the quadratic forms from are given below. Students are not expected to sketch quadratic surfaces.   Graphs of the two quadratic curves Q_A and Q_B.       Example 2  As we will see shortly, properties of the symmetric matrix used to generate the quadratic form will allow us to determine characterisitcs about the quadratic form, such as whether it is always nonnegative. Therefore, if first presented with the quadratic form, it is useful to find the symmetric matrix which generates the quadratic form.    Write the quadratic form below in the form for .     We have that satisfies .     Note in the above example it is difficult to determine whether is always nonnegative. This is due to the cross-product terms, terms of the form , since these terms could be positive or negative depending on the input.  To help us better determine properties of , we would like to perform a change of variables to obtain an equivalent quadratic form which no longer has any cross-product terms. Recall that we can find a symmetric matrix so that . Since is symmetric, we know by that for a diagonal matrix and orthogonal matrix .  What happens when we make the change of variables ?   Let be the symmetric matrix which satisfies . Since is symmetric, we can orthogonally diagonalize it as where is an orthogonal matrix. Let . Multiplying both sides by yields , where we used . Thus, using yields . Since is a diagonal matrix with the eigenvalues of on the diagonal, we have that .     Example 3: Removing Cross Terms   Principal Axes Theorem   If is symmetric, then there exists an orthogonal change of variables such that the quadratic form becomes , with no cross-product terms.      Use the orthogonal diagonalization of to remove cross-product terms from .     Note that . Let . Then, we will make the change of variables . Under this change of variables, note the following: . Therefore, we see that making the appropriate change of variables yields a quadratic from without any cross-product terms.      Classifying Quadratic Forms   Definiteness   A quadratic form is:   Positive definite if for all .  Negative definite if for all .  Positive semidefinite if for all .  Negative semidefinite if for all .  Indefinite if takes both positive and negative values.       Eigenvalue Classification   If is symmetric with eigenvalues , then:   is positive definite iff .   for all .   is negative definite iff .   for all .   is positive semidefinite iff .   for all .   is negative semidefinite iff .   for all .   is indefinite iff .   for some and for some .           Determine whether the inequality holds for all :     Note that the symmetric matrix for which is . We have that . Thus, the eigenvalues of are . It follows that under a suitable change of variables for all inputs . Therefore, for all inputs . Thus, is a positive semidefinite quadratic form.      "
},
{
...
},
{
  ...
  "number": "7.24",
  "title": "",
  "body": "  Compute and for the following matrices.          Note that            Note that     "
},
{
  ...
  "number": "7.25",
  "title": "",
  "body": "  Write the quadratic form below in the form for .     We have that satisfies .   "
},
{
...
},
{
  ...
  "body": " Eigenvalue Classification   If is symmetric with eigenvalues , then:   is positive definite iff .   for all .   is negative definite iff .   for all .   is positive semidefinite iff .   for all .   is negative semidefinite iff .   for all .   is indefinite iff .   for some and for some .     "
},
{
  ...
  "number": "7.30",
  "title": "",
  "body": "  Determine whether the inequality holds for all :     Note that the symmetric matrix for which is . We have that . Thus, the eigenvalues of are . It follows that under a suitable change of variables for all inputs . Therefore, for all inputs . Thus, is a positive semidefinite quadratic form.   "
},
{
  "id": "Section-7-3",
  "level": "1",
  ...
  "body": " 7.3 Constrained Optimization   7.3 Constrained Optimization   Overview   Topics   Constrained optimization as an eigenvalue problem  Distance and orthogonality constraints     Goals   Apply eigenvalues and eigenvectors to solve optimization problems subject to distance and orthogonality constraints.       Consider the quadratic form . Find the largest and smallest values of under the constraint that .    Consider the following outputs of under some typical unit vectors: These calculations show that each unit vector input is giving a particular weighted average of the coefficients and . Thus, the highest weighted average we could get is to put all the weight on (the first calculation we did). Similarly, the smallest weighted average we could get is to put all the weight on (the third calculation we did). Therefore, the maximum of under the unit vector input constraing is , and the minimum of under the constraint is .  Note this example is not hard since does not have any cross-product terms. If it did, then we would use to instead analyze an equivalent quadratic form without cross-product terms.     A coloring of the unit sphere which represents the values of Q from the previous example, where blue means low values and red means high values.      A Constrained Optimization Problem  Suppose we wish to find the maximum and minimum values of   subject to the constraint .  That is, we want   This is an example of a constrained optimization problem. We are also interested in where these extreme values are attained.  These problems are straight-forward when has no cross-product terms. What about when it does have cross-product terms?    Calculate the maximum and minimum values of   where and . Identify the points where these values are obtained.    Since has cross-product terms, we will make the change of variables to get an equivalent quadratic form without cross-product terms. Note that the unit vector constraint is preserved and stil imposed on since since is orthogonal matrix and tells us that multiplication preserves lengths.  We have that the symmetric matrix so that is . In , we saw that the eigenvalues are , , and with accompanying unit eigenvectors . Therefore, setting and making the change of variables , we deduce . Thus, the maximum value of under the constraint that the input is a unit vector is , and the minimum is . These maximum and minimum values occur at eigenvectors corresponding to the given eigenvalue. For example, note that       The image below is the unit sphere whose surface is colored according to the quadratic form from . Notice the agreement between our solution and the image.   A coloring of the unit sphere which represents the values of Q from the previous example, where blue means low values and red means high values.    Constrained Optimization and Eigenvalues   Extrema via Eigenvalues   Let , where is a real symmetric matrix with eigenvalues   and corresponding normalized eigenvectors   Under the constraint :   The maximum value of is , attained at .  The minimum value of is , attained at .       Calculate the maximum value of , with , subject to the constraint . Identify a point where this maximum is obtained.     Note that if , then for some unit vector . It follows that . Therefore, we have that the maximum of under the constraint that is times the maximum of under the constraint that , and the maximum is stil obtained at the appropriate eigenvector. Therefore, the maximum is and is obtained at any length vector in the -eigenspace , and the minimum is and is obtained at any length vector in the -eigenspace .       An Orthogonality Constraint   Extrema with an Orthogonality Condition   Let , where is a real symmetric matrix with eigenvalues   and corresponding eigenvectors   Subject to the constraints and :   The maximum value of is , attained at an eigenvector associated to .  The minimum value of is , attained at .   Here denotes the second largest eigenvalue of .      Calculate the maximum and minimum values of , with , subject to and , where . Identifu points where the maximum and minimum values occur. The eigenvalues of are .    Since is an eigenvector corresponding to , the maximum of under the constraint that and is the second largest eigenvalues . To find a vector that gives this new maximum, we find a unit eigenvector corresponding to . Note that . Thus, is a unit eigenvector corresponding to . Note that this vector is also orthogonal to which is guaranteed by . It follows that .      "
},
{
...
},
{
  ...
  "number": "7.31",
  "title": "",
  "body": "  Consider the quadratic form . Find the largest and smallest values of under the constraint that .    Consider the following outputs of under some typical unit vectors: These calculations show that each unit vector input is giving a particular weighted average of the coefficients and . Thus, the highest weighted average we could get is to put all the weight on (the first calculation we did). Similarly, the smallest weighted average we could get is to put all the weight on (the third calculation we did). Therefore, the maximum of under the unit vector input constraing is , and the minimum of under the constraint is .  Note this example is not hard since does not have any cross-product terms. If it did, then we would use to instead analyze an equivalent quadratic form without cross-product terms.   "
},
{
...
},
{
  ...
  "number": "7.32",
  "title": "",
  "body": "  Calculate the maximum and minimum values of   where and . Identify the points where these values are obtained.    Since has cross-product terms, we will make the change of variables to get an equivalent quadratic form without cross-product terms. Note that the unit vector constraint is preserved and stil imposed on since since is orthogonal matrix and tells us that multiplication preserves lengths.  We have that the symmetric matrix so that is . In , we saw that the eigenvalues are , , and with accompanying unit eigenvectors . Therefore, setting and making the change of variables , we deduce . Thus, the maximum value of under the constraint that the input is a unit vector is , and the minimum is . These maximum and minimum values occur at eigenvectors corresponding to the given eigenvalue. For example, note that    "
},
{
...
},
{
  ...
  "number": "7.34",
  "title": "",
  "body": "  Calculate the maximum value of , with , subject to the constraint . Identify a point where this maximum is obtained.     Note that if , then for some unit vector . It follows that . Therefore, we have that the maximum of under the constraint that is times the maximum of under the constraint that , and the maximum is stil obtained at the appropriate eigenvector. Therefore, the maximum is and is obtained at any length vector in the -eigenspace , and the minimum is and is obtained at any length vector in the -eigenspace .   "
},
{
...
},
{
  ...
  "number": "7.36",
  "title": "",
  "body": "  Calculate the maximum and minimum values of , with , subject to and , where . Identifu points where the maximum and minimum values occur. The eigenvalues of are .    Since is an eigenvector corresponding to , the maximum of under the constraint that and is the second largest eigenvalues . To find a vector that gives this new maximum, we find a unit eigenvector corresponding to . Note that . Thus, is a unit eigenvector corresponding to . Note that this vector is also orthogonal to which is guaranteed by . It follows that .   "
},
{
  "id": "Section-7-4",
  "level": "1",
...
},
{
  ...
  "number": "7.40",
  "title": "",
  "body": "  Construct the SVD of     First, we calculate the symmetric matrix . Note that .  Next, we find the eigenvalues of . Note that . Therefore, the eigenvalues of are and .  Next, we find the singular values of which are the square roots of the eigenvalues of . Therefore, we have that and . It follows that .  Next, we find corresponding eigenvectors for each eigenvalue of . Note that . Thus, is a unit eigenvector of corresponding to . Also, note that . It follows that is a unit eigenvector of corresponding to . (Note that we could have found in an alternative way by using .) These eigenvectors are the rows of . Therefore, we have that .  Lastly, we find the columns of by using the formula . Note that and . Therefore, we have that .  We can check our Singular Value Decomposition by verifying that the matrices and have the appropriate properties and satisfy . Note that    "
},
{
  "id": "ex-7-4-example2",
  "level": "2",
  ...
  "number": "7.41",
  "title": "",
  "body": "  Construct the SVD of     First, note that . We have that . So, the eigenvalues of are and . It follows that the singular values of are and . Recall that has the same dimensions as . Thus, . Since there is only one nonzero singular value, we have that the rank of is 1.  Note that . Therefore, is a unit eigenvector of corresponding to an eigenvalue of . Also, note that . Therefore, is a unit eigenvector of corresponding to an eigenvalue of . it follows that .  We can find using the formula since . Note that . Note that we cannot use the formula to find since that would require us to divide by . However, we can find by using the properties of . Recall is a  orthogonal matrix. So, we actually need to find two more columns of . Thankfully, we can choose any two unit vectors as long as they make an orthogonal matrix.  One set of vectors could be . Let's check that the matrix with these columns, call it , works. Note that   The previous choice of and are not the only vectors that could fill the columns of . Suppose instead we chose as the columns of , call the matrix . Then, we also have that is a valid Singular Value Decomposition of . There are many valid factorizations when there is more than one missing column of .   "
},
{
...
},
{
  ...
  "number": "7.43",
  "title": "",
  "body": "  Let for . Find an orthonormal basis for each of the four subspaces.    Since has three nonzero elements on its diagonal, we know that . Therefore, the first 3 columns of form an orthonormal basis for and the remaining column of forms an orthonormal basis for . Similarly, the first 3 rows of form an orthonormal basis for and the remaining rows of form an orthonormal basis for . Therefore, we have that    "
},
{
...
},
{
  ...
  "number": "7.45",
  "title": "",
  "body": "  Find the spectral decomposition of . Use the decomposition to identify the best rank 1 approximation of .    Using the information from the solution to ,we have that . Therefore, the best rank 1 matrix that approximates is .   "
},
{
